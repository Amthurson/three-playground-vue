<template>
  <div class="wrap">
    <div class="detech-result-container">
      <HandsTracking @onResult="onFaceResult" />
    </div>
    <!-- 视窗 -->
...
import HandsTracking from "@/components/AI/HandsTracking.vue"
...
export default  defineComponent({
  ...
  setup() {
    ...
    const renderHandsMesh = (res) => {
      const { handsModels,handsLines,handsPoints } = state;
      if(!res || res.length===0) {
        state.handsModels = [];
        state.handsLines = [];
        state.handsPoints = [];
      ...
      if(res.length < state.handsLines.length ) { // 根据检测结果删除痕迹模型、线、点
        const delcounts = handsLines.length - res.length;
        const delFacesModel = handsModels.splice(res.length-1,delcounts);
        delFacesModel.map(v=>{
          v.handConnectionsModel && state.scene.remove(v.handConnectionsModel)
        })
        
        const delhandLines = handsLines.splice(res.length-1,delcounts);
        delhandLines.map(v=>{
          v.handConnectionsLine && state.scene.remove(v.handConnectionsLine)
        })

        const delhandPoints = handsPoints.splice(res.length-1,delcounts);
        delhandPoints.map(v=>{
          v.handConnectionsPoint && state.scene.remove(v.handConnectionsPoint)
        })
        state.handsModels = handsModels;
        state.handsLines = handsLines;
        state.handsPoints = handsPoints;
      }
      // 绘制盒子范围
      const { aiWrap } = state;
      ...
      // 检测结果建模
      res.map((hands,i)=>{
        const { handConnectionsPoints } = hands
        if(!handConnectionsPoints || handConnectionsPoints.length===0) return;

        // 画点
        const curHandsPoints = state.handsPoints[i];
        const handConnectionsPoint = drawPoint(handConnectionsPoints.flat(),aiWrap,"#ffffff");
        const handsPoint = {
          handConnectionsPoint
        }
        if(curHandsPoints) {
          curHandsPoints.handConnectionsPoint && state.scene.remove(curHandsPoints.handConnectionsPoint);
        }
        state.handsPoints[i] = handsPoint;
        if(!handConnectionsPoint) {
          state.scene.remove(curHandsPoints.handConnectionsPoint); 
          return;
        }
        handConnectionsPoint && state.scene.add(handConnectionsPoint);

        // 划线
        const curHandsLines = state.handsLines[i];
        const handConnectionsLine = drawLine(handConnectionsPoints.flat(),aiWrap);
        const handsLine = {
          handConnectionsLine
        }
        if(curHandsLines) {
          curHandsLines.handConnectionsLine && state.scene.remove(curHandsLines.handConnectionsLine);
        }
        state.handsLines[i] = handsLine;
        if(!handConnectionsLine) {
          state.scene.remove(curHandsLines.handConnectionsLine); 
          return;
        }
        handConnectionsLine && state.scene.add(handConnectionsLine);

        // 画面
        // const curHandsModel = state.handsModels[i];
        // const handConnectionsModel = drawMesh(handConnectionsPoints.flat(),["#0050ff","#b40a00"],aiWrap); //网格模型
        ...
        // if(curHandsModel) {
        //   curHandsModel.handConnectionsModel && state.scene.remove(curHandsModel.handConnectionsModel);
        // }

        // state.handsModels[i] = handsModels;

        // if(!handConnectionsModel) {
        //   state.scene.remove(curHandsModel.handConnectionsModel); 
        //   return;
        // }
    ...
    // 画点
    const drawPoint = (faceLandMarks,aiWrap,color="#0050ff",direction= -1) => {
      ...
      [
        ...
        (-v.z*aiWrap.depth*1.2)*direction || 0]:[0,0,0]);
      const pointMaterial = new PointsMaterial({
        color: new Color(color),
        size:1
      })
