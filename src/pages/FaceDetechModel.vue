<template>
  <div class="wrap">
    <div @click="handleFullScreen" ref="detechResultRef" class="detech-result-container">
      <FaceTracking @onResult="onFaceResult" />
    </div>
    <!-- 视窗 -->
...
let initPos = null; // 初次点击位置

import FaceTracking from "@/components/AI/FaceTracking.vue"
...
export default  defineComponent({
  ...
  setup() {
    ...
    const state = reactive({
      ...
      facesLines: [], // 面部线集合
      facesPoints: [], // 面部点集合
      isDetechResultFullScreem: false, // 全屏
    })
    const init = () => {
        // 创建场景 ------------------------------------------------------------------------------------
    ...
    const click = () => {
      // 初始位置
      containerRef.value.addEventListener("pointerdown", (event) => {
      ...
      containerRef.value.addEventListener('pointerup', onDocumentMouseDown);
    }
    ...
    const renderFaceMesh = (res) => {
      const { facesModels,facesLines,facesPoints } = state;
      if(!res || res.length===0) {
        state.facesModels = [];
        state.facesLines = [];
        state.facesPoints = [];
      ...
      if(res.length < state.facesModels.length ) { // 根据检测结果删除痕迹模型
        const delcounts = facesModels.length - res.length;
        const delFacesModel = facesModels.splice(res.length-1,delcounts);
        delFacesModel.map(v=>{
          v.aiFaceTesselationModel && state.scene.remove(v.aiFaceTesselationModel)
          v.aiFaceRightEyeModel && state.scene.remove(v.aiFaceRightEyeModel)
          v.aiFaceRightEyeBowModel && state.scene.remove(v.aiFaceRightEyeBowModel)
          v.aiFaceLeftEyeModel && state.scene.remove(v.aiFaceLeftEyeModel)
          v.aiFaceLeftEyeBowModel && state.scene.remove(v.aiFaceLeftEyeBowModel)
          v.aiFaceOvalModel && state.scene.remove(v.aiFaceOvalModel)
          v.aiFaceLipsModel && state.scene.remove(v.aiFaceLipsModel)
          v.aiFaceRightIrisModel && state.scene.remove(v.aiFaceRightIrisModel)
          v.aiFaceLeftIrisModel && state.scene.remove(v.aiFaceLeftIrisModel)
        });
        const delFacesLine = facesLines.splice(res.length-1,delcounts);
        delFacesLine.map(v=>{
          v.aiFaceTesselationLine && state.scene.remove(v.aiFaceTesselationLine)
          v.aiFaceRightEyeLine && state.scene.remove(v.aiFaceRightEyeLine)
          v.aiFaceRightEyeBowLine && state.scene.remove(v.aiFaceRightEyeBowLine)
          v.aiFaceLeftEyeLine && state.scene.remove(v.aiFaceLeftEyeLine)
          v.aiFaceLeftEyeBowLine && state.scene.remove(v.aiFaceLeftEyeBowLine)
          v.aiFaceOvalLine && state.scene.remove(v.aiFaceOvalLine)
          v.aiFaceLipsLine && state.scene.remove(v.aiFaceLipsLine)
          v.aiFaceRightIrisLine && state.scene.remove(v.aiFaceRightIrisLine)
          v.aiFaceLeftIrisLine && state.scene.remove(v.aiFaceLeftIrisLine)
        })
        const delFacesPoint = facesPoints.splice(res.length-1,delcounts);
        delFacesPoint.map(v=>{
          v.aiFaceTesselationPoint && state.scene.remove(v.aiFaceTesselationPoint)
          v.aiFaceRightEyePoint && state.scene.remove(v.aiFaceRightEyePoint)
          v.aiFaceRightEyeBowPoint && state.scene.remove(v.aiFaceRightEyeBowPoint)
          v.aiFaceLeftEyePoint && state.scene.remove(v.aiFaceLeftEyePoint)
          v.aiFaceLeftEyeBowPoint && state.scene.remove(v.aiFaceLeftEyeBowPoint)
          v.aiFaceOvalPoint && state.scene.remove(v.aiFaceOvalPoint)
          v.aiFaceLipsPoint && state.scene.remove(v.aiFaceLipsPoint)
          v.aiFaceRightIrisPoint && state.scene.remove(v.aiFaceRightIrisPoint)
          v.aiFaceLeftIrisPoint && state.scene.remove(v.aiFaceLeftIrisPoint)
        })
        state.facesModels = facesModels;
        state.facesLines = facesLines;
        state.facesPoints = facesPoints;
      }
      const { aiWrap } = state;
      // 绘制盒子范围
      ...
      res.map((face,i)=>{
        const { tesselationPoints,
              ...
              rightIrisPoints,
              leftIrisPoints } = face
        if(!faceOvalPoints || faceOvalPoints.length===0) return;

        // 画点
        // const curfacesPoints = state.facesPoints[i];
        // const aiFaceTesselationPoint = drawPoint(tesselationPoints.flat(),aiWrap,"#666699"); //网格模型
        // const aiFaceRightEyePoint = drawPoint(rightEyePoints.flat(),aiWrap,"#ff0aff"); //网格模型
        // const aiFaceRightEyeBowPoint = drawPoint(rightEyebowPoints.flat(),aiWrap,"#ff9922"); //网格模型
        // const aiFaceLeftEyePoint = drawPoint(leftEyePoints.flat(),aiWrap,"#ffff00"); //网格模型
        // const aiFaceLeftEyeBowPoint = drawPoint(leftEyebowPoints.flat(),aiWrap,"#ff9922"); //网格模型
        // const aiFaceOvalPoint = drawPoint(faceOvalPoints.flat(),aiWrap,"#ffffff"); //网格模型
        // const aiFaceLipsPoint = drawPoint(lipsPoints.flat(),aiWrap,"#ffff00"); //网格模型
        // const aiFaceRightIrisPoint = drawPoint(rightIrisPoints.flat(),aiWrap,"#99ff44"); //网格模型
        // const aiFaceLeftIrisPoint = drawPoint(leftIrisPoints.flat(),aiWrap,"#b40a00"); //网格模型
        // const facesPoint = {
        //   aiFaceTesselationPoint,
        //   aiFaceRightEyePoint,
        //   aiFaceRightEyeBowPoint,
        //   aiFaceLeftEyePoint,
        //   aiFaceLeftEyeBowPoint,
        //   aiFaceOvalPoint,
        //   aiFaceLipsPoint,
        //   aiFaceRightIrisPoint,
        //   aiFaceLeftIrisPoint,
        // }
        // if(curfacesPoints) {
        //   curfacesPoints.aiFaceTesselationPoint && state.scene.remove(curfacesPoints.aiFaceTesselationPoint)
        //   curfacesPoints.aiFaceRightEyePoint && state.scene.remove(curfacesPoints.aiFaceRightEyePoint)
        //   curfacesPoints.aiFaceRightEyeBowPoint && state.scene.remove(curfacesPoints.aiFaceRightEyeBowPoint)
        //   curfacesPoints.aiFaceLeftEyePoint && state.scene.remove(curfacesPoints.aiFaceLeftEyePoint)
        //   curfacesPoints.aiFaceLeftEyeBowPoint && state.scene.remove(curfacesPoints.aiFaceLeftEyeBowPoint)
        //   curfacesPoints.aiFaceOvalPoint && state.scene.remove(curfacesPoints.aiFaceOvalPoint)
        //   curfacesPoints.aiFaceLipsPoint && state.scene.remove(curfacesPoints.aiFaceLipsPoint)
        //   curfacesPoints.aiFaceRightIrisPoint && state.scene.remove(curfacesPoints.aiFaceRightIrisPoint)
        //   curfacesPoints.aiFaceLeftIrisPoint && state.scene.remove(curfacesPoints.aiFaceLeftIrisPoint)
        // }
        // state.facesPoints[i] = facesPoint;
        // Object.entries(facesPoint).map(([key,val])=>{
        //   val && state.scene.add(val);
        // });

        // 划线
        const curfacesLines = state.facesLines[i];
        // const aiFaceTesselationLine = drawLine(tesselationPoints.flat(),aiWrap,"#ffffff"); //网格模型
        const aiFaceRightEyeLine = drawLine(rightEyePoints.flat(),aiWrap,"#ff0aff"); //网格模型
        const aiFaceRightEyeBowLine = drawLine(rightEyebowPoints.flat(),aiWrap,"#ff9922"); //网格模型
        const aiFaceLeftEyeLine = drawLine(leftEyePoints.flat(),aiWrap,"#ffff00"); //网格模型
        const aiFaceLeftEyeBowLine = drawLine(leftEyebowPoints.flat(),aiWrap,"#ff9922"); //网格模型
        // const aiFaceOvalLine = drawLine(faceOvalPoints.flat(),aiWrap,"#ffffff"); //网格模型
        const aiFaceLipsLine = drawLine(lipsPoints.flat(),aiWrap,"#ffff00"); //网格模型
        const aiFaceRightIrisLine = drawLine(rightIrisPoints.flat(),aiWrap,"#99ff44"); //网格模型
        const aiFaceLeftIrisLine = drawLine(leftIrisPoints.flat(),aiWrap,"#b40a00"); //网格模型
        const facesLine = {
          // aiFaceTesselationLine,
          aiFaceRightEyeLine,
          aiFaceRightEyeBowLine,
          aiFaceLeftEyeLine,
          aiFaceLeftEyeBowLine,
          // aiFaceOvalLine,
          aiFaceLipsLine,
          aiFaceRightIrisLine,
          aiFaceLeftIrisLine,
        ...
        if(curfacesLines) {
          // curfacesLines.aiFaceTesselationLine && state.scene.remove(curfacesLines.aiFaceTesselationLine)
          curfacesLines.aiFaceRightEyeLine && state.scene.remove(curfacesLines.aiFaceRightEyeLine)
          curfacesLines.aiFaceRightEyeBowLine && state.scene.remove(curfacesLines.aiFaceRightEyeBowLine)
          curfacesLines.aiFaceLeftEyeLine && state.scene.remove(curfacesLines.aiFaceLeftEyeLine)
          curfacesLines.aiFaceLeftEyeBowLine && state.scene.remove(curfacesLines.aiFaceLeftEyeBowLine)
          // curfacesLines.aiFaceOvalLine && state.scene.remove(curfacesLines.aiFaceOvalLine)
          curfacesLines.aiFaceLipsLine && state.scene.remove(curfacesLines.aiFaceLipsLine)
          curfacesLines.aiFaceRightIrisLine && state.scene.remove(curfacesLines.aiFaceRightIrisLine)
          curfacesLines.aiFaceLeftIrisLine && state.scene.remove(curfacesLines.aiFaceLeftIrisLine)
        }
        state.facesLines[i] = facesLine;
        Object.entries(facesLine).map(([key,val])=>{
          val && state.scene.add(val);
        });

        // 画面
        const curFaceModel = state.facesModels[i];
        const aiFaceTesselationModel = drawMesh(tesselationPoints.map(v=>v[0]),["#996666","#666699"],aiWrap,true); //网格模型
        const aiFaceRightEyeModel = drawMesh(rightEyePoints.map(v=>v[0]),["#ff50ff","#ff0aff"],aiWrap,false,3); //网格模型
        const aiFaceRightEyeBowModel = drawMesh(rightEyebowPoints.map(v=>v[0]),["#00ff22","#ff9922"],aiWrap,false); //网格模型
        const aiFaceLeftEyeModel = drawMesh(leftEyePoints.map(v=>v[0]),["#ffffff","#ffff00"],aiWrap,false,3); //网格模型
        const aiFaceLeftEyeBowModel = drawMesh(leftEyebowPoints.map(v=>v[0]),["#ffff33","#ff9922"],aiWrap,false); //网格模型
        const aiFaceOvalModel = drawMesh(faceOvalPoints.map(v=>v[0]),["#ffffff","#ffffff"],aiWrap,true); //网格模型
        const aiFaceLipsModel = drawMesh(lipsPoints.map(v=>v[0]),["#33ffff","#ffff00"],aiWrap,false,3); //网格模型
        const aiFaceRightIrisModel = drawMesh(rightIrisPoints.map(v=>v[0]),["#22ff99","#99ff44"],aiWrap,false); //网格模型
        const aiFaceLeftIrisModel = drawMesh(leftIrisPoints.map(v=>v[0]),["#0050ff","#b40a00"],aiWrap,false); //网格模型
        
        // const aiFaceTesselationModel = addSurface(tesselationPoints,aiWrap);

        const faceModel = {
          aiFaceTesselationModel, // ai 面部模型
          aiFaceRightEyeModel, // ai 面部模型
          aiFaceRightEyeBowModel, // ai 面部模型
          aiFaceLeftEyeModel, // ai 面部模型
          aiFaceLeftEyeBowModel, // ai 面部模型
          aiFaceOvalModel, // ai 面部模型
          aiFaceLipsModel, // ai 面部模型
          aiFaceRightIrisModel, // ai 面部模型
          aiFaceLeftIrisModel, // ai 面部模型
        }

        if(curFaceModel) {
          curFaceModel.aiFaceTesselationModel && state.scene.remove(curFaceModel.aiFaceTesselationModel)
          curFaceModel.aiFaceRightEyeModel && state.scene.remove(curFaceModel.aiFaceRightEyeModel)
          curFaceModel.aiFaceRightEyeBowModel && state.scene.remove(curFaceModel.aiFaceRightEyeBowModel)
          curFaceModel.aiFaceLeftEyeModel && state.scene.remove(curFaceModel.aiFaceLeftEyeModel)
          curFaceModel.aiFaceLeftEyeBowModel && state.scene.remove(curFaceModel.aiFaceLeftEyeBowModel)
          curFaceModel.aiFaceOvalModel && state.scene.remove(curFaceModel.aiFaceOvalModel)
          curFaceModel.aiFaceLipsModel && state.scene.remove(curFaceModel.aiFaceLipsModel)
          curFaceModel.aiFaceRightIrisModel && state.scene.remove(curFaceModel.aiFaceRightIrisModel)
          curFaceModel.aiFaceLeftIrisModel && state.scene.remove(curFaceModel.aiFaceLeftIrisModel)
        }

        state.facesModels[i] = faceModel;

        aiFaceTesselationModel && state.scene.add(aiFaceTesselationModel);
        aiFaceRightEyeModel && state.scene.add(aiFaceRightEyeModel);
        aiFaceRightEyeBowModel && state.scene.add(aiFaceRightEyeBowModel);
        aiFaceLeftEyeModel && state.scene.add(aiFaceLeftEyeModel);
        aiFaceLeftEyeBowModel && state.scene.add(aiFaceLeftEyeBowModel);
        aiFaceOvalModel && state.scene.add(aiFaceOvalModel);
        aiFaceLipsModel && state.scene.add(aiFaceLipsModel);
        aiFaceRightIrisModel && state.scene.add(aiFaceRightIrisModel);
        aiFaceLeftIrisModel && state.scene.add(aiFaceLeftIrisModel);
      })
    }

    // NURBS surface
    // const addSurface = (faceLandMarks,aiWrap) => {
    ...
    //     const nsControlPoints = verticesPoints.map(v=>new Vector4(v[0],v[1],v[2],1));
    //     let group = new Group();
    //     group.position.y = 50;
    //     const degree1 = 2;
    //     const degree2 = 3;
    ...
    // 画线
    const drawLine = (faceLandMarks,aiWrap,color="#0050ff") => {
      if(faceLandMarks.includes(undefined)) return null;
      const verticesPoints = faceLandMarks.map(v=>v ?
      [
        v.x*aiWrap.width-aiWrap.width*0.5 || 0,
    ...
    const drawMesh = (faceLandMarks,colorRange=["#0050ff",
        "#b40a00"],aiWrap,wireframe=false,wireframeLinewidth=1) => {
      if(faceLandMarks.includes(undefined)) return null;
      const verticesPoints = faceLandMarks.map(v=>v ?
      [
        v.x*aiWrap.width-aiWrap.width*0.5 || 0,
