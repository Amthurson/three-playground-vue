...
export default  defineComponent({
  ...
  setup() {
    ...
    const addEarth = () => {
      ...
      // 创建地球
      const earthBall = new Mesh(new SphereGeometry(150, 250, 250), new MeshStandardMaterial({
        map: textureLoader.load("/earthTexture/earth3.jpg"),roughness: 0.5,
        metalness: 0.3,
      }));
      earthBall.layers.set(0);
      state.earthBall = earthBall
      //添加到场景
      state.scene.add(earthBall);
    }
