<template>
  <div class="wrap">
    ...
    <!-- <iframe src="http://10.52.48.45:3000" ref="frameRef" frameborder="0" width="1920" height="1080" style="position:fixed"></iframe> -->
    <!-- 视窗 -->
    ...
    <!-- 信息窗口 -->
    <div style="display:none" class="infoPopwindow">
      <div class="tree" v-if="objInfo.tree">
        <treeselect v-model="objInfo.value" :multiple="true" :options="objInfo.tree">
        </treeselect>
      </div>
...
export default  defineComponent({
  ...
  setup() {
    ...
    const state = reactive({
      ...
      //   },
      //   func: (x,a,b,c) => {
      //     // console.log(x,a,b,c,Math.pow((1-Math.pow(x/a,2))/Math.pow(b,2),0.5))
      //     return a>=b && b >0 ? Math.pow((1-Math.pow(x/a,2))/Math.pow(b,2),0.5) : 0;
      //   }
      // },
      ParaboloidA: 0.5, // 函数中的a：y=ax²+bx+c （a）
    ...
    const init = () => {
        ...
        // 创建相机 ------------------------------------------------------------------------------------
        const camera = new PerspectiveCamera(75, window.innerWidth/ window.innerHeight,0.1,1000); // 创建镜头
        ...
        // 添加正方体
        // state.resPoints = getMatrics(10,10,10,10);
    ...
    // 正方体阵列
    const getMatrics = (width,length,height,gap=1) => {
      const points = [];
      for(let i=0;i<width;i++) {
        for(let j=0;j<length;j++) {
          for(let k=0;k<height;k++) {
            points.push([i*gap,j*gap,k*gap]);
          }
        }
      }
      return points;
